...
struct PLAYER_NAME : public Player {
   ...
   */
   ...
   //retorna el vei de a en funcio de i (0->dalt,1->dreta...)
   Pos neighbour(Pos a, int i){
     if(i == 0)return Pos(a.i-1,a.j);
     if(i == 1)return Pos(a.i,a.j+1);
     if(i == 2)return Pos(a.i+1,a.j);
     else return Pos(a.i,a.j-1);
   }

   //retorna el pes de la cel·la a
   int costcell(Pos a){
     if(cell(a).type == Cave or cell(a).type == Outside)return 1;
     if(cell(a).type == Rock)return 1 + cell(a).turns;
     else return 1e9;
   }
     ...
     while(not q.empty()){
       ...
       if(pos_ok(current) and cost_cu == cost[current.i*60+current.j]){
         for(int i = 0; i < 4; ++i){
           ...
           if(pos_ok(aux)){
             int ncost = cost_cu + costcell(aux);
             if(ncost < cost[aux.i*60+aux.j]){
               cost[aux.i*60+aux.j] = ncost;
               q.push(make_pair(aux,ncost));
               came_from[aux.i*60+aux.j] = current;
             }
           }
         }
       }
   ...
   void escape_balrog(Pos a){
     ...
     if(a.i < 59)lim1 = a.i + 3;
     else lim1 = 60;
     if(a.j < 59)lim2 = a.j + 3;
     else lim2 = 60;
     int id = cell(a).id;
     for(int p = i; p < lim1; ++p){
       for(int q = j; q < lim2; ++q){
